...
- **PLAN-ONLY kostnadsgaranti:** enda kommandon som körs, någonsin, i denna plan är `fmt`, `init -backend=false`, `validate` och `plan` — aldrig `apply`. **Ingen AWS-kostnad.** `validate` kräver inga credentials; `plan` görs mot ett riktigt AWS-konto (read-only, skapar inget) och är beviset på att modulen faktiskt "använder AWS" och inte bara är syntaktiskt korrekt HCL.
